...
def encrypt_mobile(mobile: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"生成salt失败: {e}")
            salt = ""

        # 第一层MD5: "ZT" + mobile + "ZT"
        md5_value = hashlib.md5(f"ZT{mobile}ZT".encode("utf-8")).hexdigest()

        # 第二层MD5: salt + "ztinfo"
        salt_value = hashlib.md5(f"{salt}ztinfo".encode("utf-8")).hexdigest()

        # 第三层MD5: md5_value + salt_value
        result = hashlib.md5(f"{md5_value}{salt_value}".encode("utf-8")).hexdigest()

        logger.info(f"手机号加密成功: {mobile[:3]}****{mobile[-4:]}")
        return result

    except Exception as e:
        logger.error(f"手机号加密失败: {e}")
        raise ValueError(f"手机号加密失败: {str(e)}")
