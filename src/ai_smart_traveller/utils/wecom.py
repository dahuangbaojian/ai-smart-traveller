...
class WeComService:
    def __init__(self):
        ...
        self._cache_expires = 3600  # 缓存1小时
    ...
    def decrypt_message(
        ...
        """解密消息"""
        try:
            return self.crypto.decrypt_message(encrypt_msg, msg_signature, timestamp, nonce)
        except Exception as e:
    ...
    def parse_message(
        ...
        try:
            ...
            if msg_type == "event":
                ...
                agent_id = (
                    msg_root.find("AgentID").text if msg_root.find("AgentID") is not None else None
                )
            ...
            # 处理图片消息
                ...
                pic_url = (
                    msg_root.find("PicUrl").text if msg_root.find("PicUrl") is not None else None
                )
                media_id = (
                    msg_root.find("MediaId").text if msg_root.find("MediaId") is not None else None
                )
                return {
                    "msg_type": msg_type,
                    "pic_url": pic_url,
    ...
    def get_user_info(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            # 先尝试从Redis缓存获取
            cache_key = f"WECOM_USER_INFO:{user_id}"
            ...
            if cached_data:
                try:
                    ...
                    logger.warning(f"缓存数据格式错误: {user_id}")

            # 缓存未命中，从API获取
            access_token = self.get_access_token()
            url = f"{self.api_url}/user/get?access_token={access_token}&userid={user_id}"
            ...
            if result["errcode"] == 0:
                ...
                # 缓存到Redis
                self.redis_client.setex(cache_key, self._cache_expires, json.dumps(result))
                return result
            else:
    ...
    async def download_media_file(self, media_id: str, file_path: str) -> bool:
        ...
        try:
            ...
            # 企业微信媒体文件下载API
            download_url = (
                f"{self.api_url}/media/get?access_token={access_token}&media_id={media_id}"
            )
    ...
    def get_department_info(self, department_id: int) -> Dict[str, Any]:
        ...
        try:
            # 先尝试从Redis缓存获取
            cache_key = f"WECOM_DEPT_INFO:{department_id}"
            cached_data = self.redis_client.get(cache_key)
            if cached_data:
                try:
                    dept_info = json.loads(cached_data)
                    logger.debug(f"从缓存获取部门信息: {department_id}")
                    return dept_info
                except json.JSONDecodeError:
                    logger.warning(f"缓存数据格式错误: {department_id}")
            ...
            if result["errcode"] == 0:
                ...
                # 缓存到Redis
                self.redis_client.setex(cache_key, self._cache_expires, json.dumps(dept_data))
                return dept_data
            else:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if result["errcode"] != 0:
                    ...
                    if result["errcode"] == 42001:
                        self.access_token = None
                        self.access_token_expires = 0
                        return await self._send_single_markdown_message(user_id, content)
    ...
    async def _send_split_markdown_messages(
        ...
        try:
            ...
            for paragraph in paragraphs:
                ...
                if len(current_message) + len(paragraph) + 2 > max_length and current_message:
                    message_count += 1
                    await self._send_single_markdown_message(user_id, current_message.strip())
...
💡 PDF版本更适合预览和分享"""
    ...
    async def _send_file_message(self, user_id: str, media_id: str, filename: str) -> None:
        """发送文件消息"""
        try:
            access_token = self.get_access_token()
            url = f"{self.api_url}/message/send?access_token={access_token}"
    ...
    async def send_location_message(
        self, user_id: str, latitude: str, longitude: str, title: str, address: str, scale: int = 15
    ) -> None:
        """发送位置消息
